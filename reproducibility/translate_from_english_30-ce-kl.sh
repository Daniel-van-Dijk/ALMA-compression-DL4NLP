...
model_hf="Max-Bosch/ALMA-7B-pruned-GBLM-unstructured-0.3"
model="ALMA-7B-pruned-GBLM-30percent-ce-kl-r16"
...
for tgt in "${!languages[@]}"; do
    ...
    python translate_distilled.py \
        ...
        --ckpt "../distillation/30percent-ce-kl-r16/checkpoint-7338" \
        --src "$src" \
        --tgt "$tgt" \
        --dtype bfloat16 \
        --model "$model_hf" \
