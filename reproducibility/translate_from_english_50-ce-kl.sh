...
model_hf="Max-Bosch/ALMA-7B-pruned-GBLM-unstructured"
model="ALMA-7B-pruned-GBLM-50percent-ce-kl-r16"
...
for tgt in "${!languages[@]}"; do
    ...
    python translate_distilled.py \
        ...
        --ckpt "../distillation/50percent-ce-kl-r16/checkpoint-7338" \
        --src "$src" \
        --tgt "$tgt" \
        --dtype bfloat16 \
        --model "$model_hf" \
